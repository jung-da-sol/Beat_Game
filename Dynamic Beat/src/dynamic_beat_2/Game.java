...
public class Game extends Thread {
	...
	ArrayList<Note> noteList = new ArrayList<Note>();  //note떨어질 때 마다 관리해주는 
	...
	public void screenDraw(Graphics2D g) {
		...
		g.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON); //글자깨짐 방지
		g.setColor(Color.white);
		g.setFont(new Font("Arial", Font.BOLD, 30));  // 글자
	...
	public void dropNotes(String titleName) {   //각각 노트들이 떨어지는 것
		Beat[] beats = null;
		if(titleName.equals("Joakim Karud - Mighty Love") && difficulty.equals("Easy")) {
			int startTime = 4460 - Main.REACH_TIME * 1000;
			int gap = 125;   //박자에 맞게 떨어뜨리기 위해 계산
			beats = new Beat[] {
		...
		} else if(titleName.equals("Wild Flower - Joakim Karud") && difficulty.equals("Easy")) {
			int startTime = 1000 - Main.REACH_TIME * 1000;
			beats = new Beat[] {
					new Beat(startTime, "space")
			
			};
		} else if(titleName.equals("Wild Flower - Joakim Karud") && difficulty.equals("Hard")) {
			int startTime = 1000 - Main.REACH_TIME * 1000;
			beats = new Beat[] {
					new Beat(startTime, "space")
